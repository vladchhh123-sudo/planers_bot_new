...
# --- ИСПРАВЛЕННЫЕ ИМПОРТЫ (БЕЗ ТОЧЕК) ---
...
async def on_chat_join_request(join_request: ChatJoinRequest) -> None:
    ...
    logger.info(f"Заявка получена от {join_request.from_user.id}")
...
async def check_channel_access_handler(callback: CallbackQuery) -> None:
    ...
    if not has_access:
        await callback.answer("Заявка не найдена. Сначала подай заявку в канал!", show_alert=True)
